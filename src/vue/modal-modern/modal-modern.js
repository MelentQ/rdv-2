...
class ModalModern {
    constructor(el, selector, params = {}) {
        ...
        const defaultParams = {
            tingle: {
                ...
                onOpen: () => {
                    if (this.root)
                        this.root.dispatchEvent(new CustomEvent('modal:open'));
                },
                onClose: () => {
                    if (this.root)
                        this.root.dispatchEvent(new CustomEvent('modal:close'));
        ...
        if (this.root)
            elementParams = this.root.getAttribute(this.selector);
    ...
    openModal() {
        ...
        if (this.params.useUrlHash && this.root) {
            if (window.location.hash !== this.root.dataset.form)
                history.pushState('', document.title, '#' + this.root.dataset.form);
        }
...
class ModalModernFactory {
    constructor(props) {
        ...
        [...document.querySelectorAll(`[${props.selector}]`)].forEach(el => {
            this.modules.push(
                new ModalModern(el, props.selector, props.params)
            );
        });
