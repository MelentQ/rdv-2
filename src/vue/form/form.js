...
class Form {
	...
	_initCompanyAndBase() {
		...
		companyName.addEventListener('change', () => {
			if(!BaseNameChange) {
				...
				baseName.value = this.rus_to_latin(textData.toLowerCase().replaceAll(' ', '-').replace(/(-)\1{1,}/g, "$1").replace(/[^a-zа-яё0-9-]/gi, ''));
	...
	async _onFormSubmit(token) {
		...
		if (this.params.method !== 'get') {
			...
			if (this.params.calculatorForm && window.calculatorApp?.$children[0]?.totalData) formData.set('commentary', window.calculatorApp?.$children[0]?.totalData);
		}
